...
-- Функция для проверки, нужно ли удалять объект
function ShouldRemoveObject(obj, objectName)
    -- Исключение для Glowe и Hat в Groundskeeper
    if (objectName == "Glowe" or objectName == "Hat") and IsPartOfGroundskeeper(obj) then
        return false
    end
    
    -- Правила для lightning
    if obj.Parent and obj.Parent.Name == "lightning" then
        if obj.Name == "Sky" or obj.Name == "XBoxColor" then
            return false  -- Не удалять Sky и XBoxColor в lightning
        else
            return true   -- Удалять всё остальное в lightning
        end
    end
    
    -- Правила для TrickOrTreatHouse
    if obj.Parent and obj.Parent.Name == "TrickOrTreatHouse" then
        if obj.Name == "Void" or obj.Name == "HouseDoor" or obj.Name == "Neon" then
            return false  -- Не удалять Void, HouseDoor и Neon в TrickOrTreatHouse
        else
            return true   -- Удалять всё остальное в TrickOrTreatHouse
        end
    end
...
function CheckAndRemoveObjectsInWorkspace()
        ...
        if #objects > 0 then
            for _, obj in ipairs(objects) do
                ...
                if ShouldRemoveObject(obj, objectName) then
                    ...
                    -- Логируем исключения для отладки
                    if obj.Parent and (obj.Parent.Name == "lightning" or obj.Parent.Name == "TrickOrTreatHouse") then
                        warn("⚠️ Объект " .. objectName .. " защищен от удаления в " .. obj.Parent.Name)
                    else
    ...
    -- Дополнительная очистка для lightning (все объекты кроме Sky и XBoxColor)
    local lightningFolder = game:GetService("Workspace"):FindFirstChild("lightning")
    if lightningFolder then
        for _, child in ipairs(lightningFolder:GetChildren()) do
            if child.Name ~= "Sky" and child.Name ~= "XBoxColor" then
                if SafeDestroy(child) then
                    objectsRemoved = true
                    warn("🗑️ Удален объект из lightning: " .. child.Name)
                end
            end
        end
    end
    
    -- Дополнительная очистка для TrickOrTreatHouse (все объекты кроме Void, HouseDoor и Neon)
    local trickOrTreatHouse = game:GetService("Workspace"):FindFirstChild("TrickOrTreatHouse")
    if trickOrTreatHouse then
        for _, child in ipairs(trickOrTreatHouse:GetChildren()) do
            if child.Name ~= "Void" and child.Name ~= "HouseDoor" and child.Name ~= "Neon" then
                if SafeDestroy(child) then
                    objectsRemoved = true
                    warn("🗑️ Удален объект из TrickOrTreatHouse: " .. child.Name)
                end
            end
        end
    end
...
function MonitorNewObjectsInWorkspace()
    ...
    game:GetService("Workspace").DescendantAdded:Connect(function(descendant)
        ...
        if isInWorkspace then
            -- Проверка объектов из основного списка
            for _, objectName in ipairs(objectsToRemove) do
                if descendant.Name == objectName then
                    ...
                    if ShouldRemoveObject(descendant, objectName) then
                        ...
                        warn("⚠️ Новый объект " .. objectName .. " защищен от удаления")
                    end
                end
            end
            
            -- Специальная обработка для lightning
            if descendant.Parent and descendant.Parent.Name == "lightning" then
                if descendant.Name ~= "Sky" and descendant.Name ~= "XBoxColor" then
                    if SafeDestroy(descendant) then
                        warn("🚨 Новый объект удален из lightning: " .. descendant.Name)
                    end
                end
            end
            
            -- Специальная обработка для TrickOrTreatHouse
            if descendant.Parent and descendant.Parent.Name == "TrickOrTreatHouse" then
                if descendant.Name ~= "Void" and descendant.Name ~= "HouseDoor" and descendant.Name ~= "Neon" then
                    if SafeDestroy(descendant) then
                        warn("🚨 Новый объект удален из TrickOrTreatHouse: " .. descendant.Name)
                    end
                end
            end
        end
...
function StartWorkspaceCleaner()
    ...
    print("⚠️ Исключения:")
    print("   - Glowe и Hat не удаляются, если являются частью Groundskeeper")
    print("   - В lightning сохраняются только Sky и XBoxColor")
    print("   - В TrickOrTreatHouse сохраняются только Void, HouseDoor и Neon")
